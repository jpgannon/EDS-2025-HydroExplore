...
overflow-y:scroll; background: ghostwhite;}")),
                 ...
                 tabPanel("Heatmap Analysis",
                          fluidPage(
                            sidebarLayout(
                              sidebarPanel(
                                checkboxInput("addprecip", "Add Precip Data", value = TRUE),
                                checkboxInput("addstreamflow", "Add Streamflow Data", value = FALSE),
                                checkboxInput("addsnow", "Add Snow Levels Data", value = FALSE),
...
server <- function(input, output, session) {
  ...
  output$heatmap <- renderPlot({
    df <- filtered_dataset() %>%
      filter(obs_date >= input$zoom_rolling[1] & obs_date <= input$zoom_rolling[2]) %>%
    ...
    data_to_plot <- if (input$addprecip) {
      df %>%
        mutate(value = precip) %>%
        group_by(year, day_of_year) %>%
        summarise(total_value = sum(value, na.rm = TRUE))
    } else if (input$addstreamflow) {
      df %>%
        mutate(value = streamflow) %>%
        group_by(year, day_of_year) %>%
        summarise(total_value = sum(value, na.rm = TRUE))
    } else if (input$addsnow) {
      df %>%
    ...
    ggplot(data_to_plot, aes(x = day_of_year, y = year, fill = total_value)) +
      ...
      labs(title = paste(input$single_watershed, "Heatmap by Day of Year and Year"),
           x = "Day of Year",
           y = "Year",
           fill = paste(if (input$addprecip) "Precipitation" else if (input$addstreamflow) "Streamflow" else "Snow Depth")) +
  ...
  output$trendPlot <- renderPlot({
    ...
    p <- ggplot(df, aes(x = as.Date(obs_date))) +
      ...
      scale_color_brewer(palette = "all_colors")
