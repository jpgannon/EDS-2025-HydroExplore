  ...
  output$monthly_summary <- renderPlotly({
    if (input$time_period == "Monthly") {
      ...
      if (input$addprecip) {
        ...
        p <- p + geom_line(aes(y = precip, color = "Precip", size = 1))
      }
      if (input$addstreamflow) {
        
        p <- p + geom_line(aes(y = streamflow, color = "Streamflow", size = 1))
      }
      if (input$addsnow) {
        
        p <- p + geom_line(aes(y = snow_depth, color = "Snow Depth", size = 1))
      }
      if (input$addprecipdischarge) {
        
        p <- p + geom_line(aes(y = precip_divided_by_discharge, color = "Precip/Streamflow", size = 1))
      }
      return(ggplotly(p))
    }
